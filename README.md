...
4. 单页条数修改后会立即写入当前页面查询参数，并重置到第 1 页。
5. 排序列或排序方向修改后会立即重排当前配置列表。
...
2. 新建用户脚本。
3. 将 `nacos-config-dataid-desc-100.user.js` 的内容复制到用户脚本编辑器中。
...
真正执行分页、排序和 UI 逻辑前，脚本还会在运行时确认当前路径为 Nacos 页面，并且 hash 路由以 `#/configurationManagement` 开头。
